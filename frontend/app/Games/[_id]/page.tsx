...
const Page: React.FC<PageProps> = ({ params }) => {
  ...
  if (selectedStory || isCreating) {
    return <StoryEditor id={params._id} story={selectedStory} />;
  }
    ...
    <div className='w-screen font-comic h-[1750px] flex flex-col'>
      <div className='h-80 border-game relative w-full flex items-center flex-col'>
        ...
        <div className='w-4/5 font-comic flex flex-col gap-10 mt-20'>
          ...
          <button onClick={handleCreateStory} className='w-full bg-black hover:opacity-80 text-center text-white rounded-3xl border h-20 text-xl md:text-2xl lg:text-3xl xl:text-4xl'>Create your Story</button>
        </div>
        ...
        <div className='w-4/5'>
          ...
          <div className='mt-4 flex flex-col gap-8'>
            {filteredStories.map((story, index) => (
              <Storytitle key={index} story={story} onReadMore={() => handleReadMore(story)} />
            ))}
          </div>
